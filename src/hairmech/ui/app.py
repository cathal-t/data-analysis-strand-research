...
Dash front-end for hair-mechanical analysis.

• Upload Dimensional & Tensile .txt files
• Edit conditions, ✓-toggle the control row
• Interactive Overlay / Violin plots
• Download Metrics.xlsx and Stats.xlsx (wide table with multi-level columns)
"""
...
def _overlay_fig(areas, tensile, conds):
    ...
    for slot, df in tensile.per_slot():
        ...
        rows.extend(
            {"Slot": slot, "Condition": slot_to_cond[slot], "Strain": st, "Stress_MPa": sp / 1e6}
...
def _violin_fig(areas, tensile, conds):
    return make_violin_grid(build_summary(areas, tensile, conds), conds)
...
def _max_slot(areas: dict[int, float], tensile: TensileTest) -> int:
    return max(max(areas, default=0), max((s for s, _ in tensile.per_slot()), default=0))


def _to_excel_bytes(sheets: dict[str, pd.DataFrame]) -> bytes:
    """
    Write multiple DataFrames to an Excel file in-memory.

    • If the DataFrame has multi-level columns (e.g. Stats wide table),
      we keep `index=True` so Pandas can write the header structure.
    • For flat tables (e.g. Metrics), we drop the index.
    """
    buf = BytesIO()
    with pd.ExcelWriter(buf, engine="xlsxwriter") as xls:
        for name, df in sheets.items():
            if isinstance(df.columns, pd.MultiIndex):
                df.to_excel(xls, sheet_name=name)            # keep index
            else:
...
def build_dash_app(root_dir: str | Path | None = None) -> Dash:
    ...
    app = Dash(__name__, title="Hair-mech", external_stylesheets=[dbc.themes.BOOTSTRAP],
    ...
    app.layout = dbc.Container(
        [
            ...
            dcc.Dropdown(id="tabs",
                         ...
                         value="overlay", clearable=False, style={"width": "250px"}),
    ...
    def _prime_table(dim_b64, ten_b64):
        ...
        return [{"name": "Condition 1", "slot_start": 1,
                 "slot_end": _max_slot(areas, tensile), "is_control": TICK}]
    ...
    def _del_rows(_, rows, sel):
        if not sel:
            raise PreventUpdate
        return [r for i, r in enumerate(rows) if i not in sel]
    ...
    def _cache(_, rows, dim_b64, ten_b64):
        ...
        for row in rows:
            ...
            seen.update(rng)
            conds.append(Condition(name, list(rng), row["is_control"] == TICK))

        if sum(c.is_control for c in conds) != 1:
            raise ConfigError("Mark exactly one ✓ row as control")

        return json.dumps({"dim_b64": dim_b64, "ten_b64": ten_b64, "conds": [asdict(c) for c in conds]})
